...
class Adapter:
    ...
    def __init__(self, root_path: str):
        ...
        # For pre-generated data, we don't need to load raw checkin files
        self.df = None
    ...
    def to_dataframes(self):
        ...
        Uses pre-generated workers.txt and tasks.txt files created by CheckinSynthesiser.py
        """
        import os
        
        workers_path = os.path.join(self.root, "workers.txt")
        tasks_path = os.path.join(self.root, "tasks.txt")
        
        if not (os.path.isfile(workers_path) and os.path.isfile(tasks_path)):
            raise FileNotFoundError(
                f"Pre-generated workers.txt and tasks.txt files not found in {self.root}. "
                f"Please run CheckinSynthesiser.py to generate these files."
            )
        ...
        workers_df["release_time"] = pd.to_datetime(workers_df["release_time"], utc=True)
        workers_df["deadline"] = pd.to_datetime(workers_df["deadline"], utc=True)
        
        tasks_df["release_time"] = pd.to_datetime(tasks_df["release_time"], utc=True)
        tasks_df["expire_time"] = pd.to_datetime(tasks_df["expire_time"], utc=True)
    ...
    def _load_checkins(self) -> pd.DataFrame:
        """
        Load every *.csv or *.txt file in `self.root` and return a single dataframe
        sorted by timestamp.
        """
        files = list(self.root.glob("*.csv")) + list(self.root.glob("*.txt"))
        if not files:
            raise FileNotFoundError(f"No .csv or .txt files found in {self.root}")
